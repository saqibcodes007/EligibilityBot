...
├── credentials.json        # Service Account credentials for Google APIs.
├── requirements.txt        # List of required Python packages.
├── login_state.json        # (Generated by the bot) Stores the browser session to avoid re-logins.
│
└── Screenshots/            # (Generated by the bot) Temporary storage for screenshots before upload.
    └── ... (screenshot files are saved here)
